...
class Command(BaseCommand):
    help = "Dump data FIN_DUMP to JSON (monthly), then delete all data from database."
    ...
    @staticmethod
    def run_dump():
        """Ambil semua FIN_DUMP bulan ini → simpan JSON → hapus DB"""
        now = datetime.datetime.now()
        month_str = now.strftime("%m")
        year_str = now.strftime("%Y")

        # ambil data dari DB alias fin_dump
        qs = FinDump.objects.using("fin_dump").filter(
            created_at__year=year_str,
            created_at__month=month_str,
        )

        if not qs.exists():
            print(f"[FIN_DUMP] There's no record for {month_str}/{year_str}")
            return None
        ...
        filename = f"fin_{month_str}.json"
        filepath = dumps_dir / filename

        with open(filepath, "w", encoding="utf-8") as f:
            json.dump(data, f, ensure_ascii=False, indent=2)

        print(f"[FIN_DUMP] Data saved successfully at {filepath}")
        ...
        count, _ = qs.delete()
        print(f"[FIN_DUMP] {count} records deleted after dump")
